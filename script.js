...
let keranjang = [];
let currentUserRole = "customer"; // "admin" atau "customer"

// 1. DUA SKENARIO LOG IN
function prosesLogin() {
  const user = document.getElementById("loginUser").value.trim().toLowerCase();
  
  if (user === "meila" || user === "admin") {
    currentUserRole = "admin";
    document.getElementById("adminSidebar").style.display = "block"; // Tampilkan Sidebar Pink
    pindahHalaman("menuEditHarga"); // Admin langsung ke menu kelola
  } else {
    currentUserRole = "customer";
    document.getElementById("adminSidebar").style.display = "none"; // Sembunyikan Sidebar Pink
    pindahHalaman("menuKatalog"); // Pelanggan langsung ke Tampilan Foto 2
  }
...
function renderCheckoutPage() {
  ...
  <button class="m-btn m-btn-confirm" style="margin-top:15px; width:100%;" onclick="prosesBayar()">Bayar Sekarang</button>`;
  ...
  bukaModalBase({
    ...
    onConfirm: () => {
      ...
      if (!isNaN(hargaBaru) && hargaBaru > 0) {
        ...
        refreshAllViews();
        tampilkanNotificationModal("Berhasil!", "Harga jual telah diperbarui.");
      }
    }
  });
}
...
function bukaFiturAkun() {
  if (currentUserRole === "admin") {
    pindahHalaman("menuEditHarga");
  } else {
    tampilkanNotificationModal("Profil Akun", "Anda login sebagai Customer.");
  }
}
