...
function checkOnboarding() {
    if (!state.user) {
        ...
        document.getElementById('profile-address').innerText = state.user.address || "Jl. Mangkang Kulon SMK Texmaco Semarang Jawa Tengah Indonesia";
...
function handleWizardStep(e) {
    ...
    if (state.wizardStep === 1) {
        ...
        document.getElementById('wizard-subtitle').innerText = "Langkah 2 dari 4: Buat Akun Anda";
        ...
        document.getElementById('wizard-subtitle').innerText = "Langkah 3 dari 4: Nomor Kontak";
        ...
        document.getElementById('wizard-subtitle').innerText = "Langkah 4 dari 4: Alamat Pengiriman";
        document.getElementById('wiz-address').required = true;
...
function updateUI() {
    ...
    // Render Orders / Status Pesanan Ala Azariya
            ...
            ordersContainer.innerHTML += `
                <div class="order-card">
                    <div class="order-header">
                        <span class="order-id">#AZR-${o.id}</span>
                        <span class="status-badge">📦 ${o.status}</span>
                    </div>
                    <div class="order-details">
                        <p>Pemesanan Oleh: <b>${o.customerName}</b> (${o.customerPhone})</p>
                        <p>Email: ${o.customerEmail}</p>
                        <p>Alamat Pengiriman: ${o.customerAddress}</p>
                        <div class="order-item-title">${o.itemsSummary} (Inc. Ongkir ${formatRupiah(o.shippingCost)})</div>
                        <div class="order-total-price">${formatRupiah(o.total)} (COD)</div>
                    </div>
                </div>
            `;
        });
...
function processCheckout() {
    ...
    const newOrder = {
        ...
        customerName: state.user ? state.user.username : 'Guest',
        customerPhone: state.user ? state.user.phone : '-',
        customerEmail: state.user ? state.user.email : '-',
        customerAddress: state.user && state.user.address ? state.user.address : 'Jl. Mangkang Kulon SMK Texmaco Semarang Jawa Tengah Indonesia',
...
function clearOrderHistory() {
    if (confirm("Apakah Anda yakin ingin menghapus seluruh riwayat status pesanan?")) {
        state.orders = [];
        saveState();
        showToast("🗑️ Riwayat pesanan berhasil dihapus!");
    }
}
