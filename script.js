// DATA BARANG DENGAN WARISAN DATA LAMA UTUH
let daftarBarang = [
...
// 2. LOGIKA KERANJANG & BELI (LANGSUNG KE CHECKOUT)
...
// 3. TABEL VIEW & EDIT HARGA SERTA BARANG (PENOMORAN URUT: 1, 2, 3...)
function renderTabelHarga() {
  ...
  daftarBarang.forEach((item, index) => {
    ...
    row.innerHTML = `
      <td><b>${index + 1}</b></td> <!-- Penomoran Urut Otomatis 1, 2, 3... -->
      <td>${item.nama}</td>
...
// 4. MODAL CUSTOM UNTUK EDIT HARGA & EDIT BARANG (BEBAS POPUP GOOGLE BROWSER)
function bukaModalEditHarga(id) {
  const barang = daftarBarang.find(b => b.id === id);
  if (!barang) return;

  // Sembunyikan field yang tidak dibutuhkan
