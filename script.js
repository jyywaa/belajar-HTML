...
function render(list,target){document.getElementById(target).innerHTML=list.map(p=>`<div class="card-cus" onclick="showDetail(${p.id})"><span class="badge">${p.diskon}</span><img src="${p.img}"><div class="info"><div style="font-weight:700">${p.nama}</div><div><span class="price">Rp${p.harga.toLocaleString()}</span> <span class="old">Rp${p.old?.toLocaleString()}</span></div><div>★ ${p.rating}</div><div class="card-actions"><span class="btn-icon" onclick="event.stopPropagation();addCart(${p.id})">🛒</span><button class="btn-beli" onclick="event.stopPropagation();addCart(${p.id})">Beli</button></div></div></div>`).join('')}
...
function navCus(tab){setCusTab(tab);document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));event.currentTarget.classList.add('active')}
